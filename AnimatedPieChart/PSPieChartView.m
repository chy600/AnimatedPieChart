...
@implementation PSPieChartView
{
    ...
    PSPieSlice *selectedSlice;
    PSPieSlice *nextSelectedSlice;
    float selectionYPos;
...
-(id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        pieChart = [[PSPieChart alloc] init];
        xPos = 320/2;
        yPos = 330;
        radius = 120;
        rotationAngle = 0;
...
- (void) notifySelectedSlice: (PSPieSlice *)slice {
...
-(void)drawPieChart:(CGContextRef) context {
    ...
    CGContextFillRect(context, CGRectMake(0, 0, 320, [UIScreen mainScreen].bounds.size.height));
    
    //Draw all slices
    float a = rotationAngle;
    for(PSPieSlice *slice in pieChart.slices) {
        [self drawPieSlice:slice withStartingAngle:a withContext:context];
        a += (slice.pct/100) * (M_PI*2);
    }
}

-(void)drawPieSlice:(PSPieSlice *)slice withStartingAngle:(float)startingAngle withContext:(CGContextRef)context {
    ...
    CGContextSetFillColorWithColor(context, [PSPieChartView colorFromHexString: slice.colour].CGColor);
